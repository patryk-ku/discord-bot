	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				.setName('cover')
				.setDescription(
					'Replies with high-res cover art of user now playing song (from listenbrainz).'
				)
				.addUserOption((option) =>
					option.setName('user').setDescription('The user (default you).')
				)
	...
	async execute(interaction) {
		if (!process.env.LISTENBRAINZ_TOKEN) {
			return interaction.reply(
				'Listenbrainz commands are **disabled** because the bot owner did not provided an Listenbrainz API token.'
			);
		}
				...
				switch (interaction.options.getSubcommand()) {
					case 'set': {
						...
						try {
							...
							return interaction.editReply(
								`Your listenbrainz login is set to: \`${row.listenbrainz}\``
							);
						} catch (error) {
							if (error.name === 'SequelizeUniqueConstraintError') {
								...
								if (affectedRows > 0) {
									return interaction.editReply(
										`Your new listenbrainz nickname is \`${nickname}\`.`
									);
								}
							}
							...
							console.log(error);
							return interaction.editReply(
								'Error: Something went wrong with setting a listenbrainz username.'
							);
						}
					}
					...
					case 'remove': {
						...
						await interaction.editReply(
							'Deleting listenbrainz username from database...'
						);
			...
			default: {
				switch (interaction.options.getSubcommand()) {
					case 'np': {
						...
						const songEmbed = await Listenbrainz.embedNowPlaying(
							user,
							listenbrainzNickname
						);
						if (songEmbed.error) {
					...
					case 'cover': {
						...
						const nowPlaying = await Listenbrainz.getNowPlaying(
							user,
							listenbrainzNickname
						);
						if (nowPlaying.error) {
						...
						if (
							!nowPlaying.payload.listens[0].track_metadata.additional_info
								.release_mbid
						) {
							return interaction.editReply({
								content:
									'The `mbid` of this track is missing in MusicBrainz database. Unable to fetch cover art image for this release.',
							});
						}
						const mbid =
							nowPlaying.payload.listens[0].track_metadata.additional_info
